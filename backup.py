...
def fit_angel_pca(sr, thr):
    ...
    for index, cnt in enumerate(contours):
        ...
        if area >= min:
            ...
            scale = 100
            vector2_end = (
            int(mean_point[0] + eigenvectors[0][0] * scale), int(mean_point[1] + eigenvectors[0][1] * scale))
            cv2.line(sr, mean_point, vector2_end, (0, 255, 0), 1, cv2.LINE_AA)
            text = f"(angel: {angel})"
            cv2.putText(sr, text, (int(round(mean[0][0]) - 30), int(round(mean[0][1]) + 30)), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, (255, 0, 0), 1)
...
#
# path = r"datafornichi\template\h11_tem.bmp"
# coordinates_src = ""
...
# result = matching(edges_src, edges_tem, template, object, 0.8, sr0)
# myst = ""
